...
class SortingEngine:
    ...
    def sort(
        ...
        def sort_key(schedule: Schedule) -> Tuple:
            ...
            return tuple(-_SCORERS[c](schedule, courses) for c in criteria)
...
# ---------------------------------------------------------------------------
# Criterion scorers — (schedule, courses) -> float (higher = ranked first)
# ---------------------------------------------------------------------------

def _score_3_1(schedule: Schedule, courses: List[Course]) -> float:
    """3.1 — min days between mandatory exams (same program, year)."""
    groups = _mandatory_dates_by_group(schedule, courses)
    if not groups:
        return 0.0
    return float(min(_min_gap(dates) for dates in groups.values()))


def _score_3_2(schedule: Schedule, courses: List[Course]) -> float:
    """3.2 — average days between any two exams (same program, year)."""
    groups = _all_dates_by_group(schedule, courses)
    avgs = [_avg_gap(dates) for dates in groups.values() if len(dates) >= 2]
    return sum(avgs) / len(avgs) if avgs else 0.0


def _score_3_3(schedule: Schedule, courses: List[Course]) -> float:
    """3.3 — total elective-elective same-day collision pairs (same program)."""
    groups = _elective_dates_by_program(schedule, courses)
    return float(sum(_count_same_day_pairs(dates) for dates in groups.values()))


def _score_3_4(schedule: Schedule, courses: List[Course]) -> float:
...
def _score_3_5(schedule: Schedule, courses: List[Course]) -> float:
