...
Test cases:
    1. Basic output — period sub-header and schedule lines.
...
def test_basic_output_format(tmp_path):
    ...
    exporter.export_schedules(
        schedules_by_period={"FALL - Aleph": [schedule]},
...
def test_spri_displays_as_spring(tmp_path):
    ...
    exporter.export_schedules(
        schedules_by_period={"SPRI - Aleph": [schedule]},
        courses_by_id={"22222": course},
    )

    content = (tmp_path / "schedules.txt").read_text(encoding="utf-8")
    assert "[SPRING - Aleph]" in content
    assert "[SPRI - Aleph]" not in content
...
def test_multiple_schedules_numbered_from_one(tmp_path):
    ...
    exporter.export_schedules(
        schedules_by_period={"FALL - Aleph": schedules},
        courses_by_id={"11111": course},
    )
...
def test_multiple_periods_cross_product(tmp_path):
    exporter = TextFileExporter(tmp_path / "schedules.txt")
    ...
    exporter.export_schedules(
        schedules_by_period={
            "FALL - Aleph": [_make_schedule(period_fall, {"11111": date(2025, 1, 5)})],
            "SPRI - Bet": [_make_schedule(period_spri, {"11111": date(2025, 3, 5)})],
        },
        courses_by_id={"11111": course},
    )

    content = (tmp_path / "schedules.txt").read_text(encoding="utf-8")
    # Cross-product of 1×1 = 1 combined schedule with both period sub-headers
    assert "Schedule #1:" in content
    assert "[FALL - Aleph]" in content
